#!/usr/bin/env python3
"""Add CartHop routes to the existing shared ingress without replacing other projects."""
import argparse
import base64
import fcntl
import hashlib
...
TARGETS = {'carthop-prod.conf.template'}
...
def verify_applied(after, expected_configs, required_networks):
    ...
    if any(actual.get(target) != config_id for target, config_id in expected_configs.items()):
        raise RuntimeError('Expected CartHop route was not installed')
...
def main():
    ...
    os.umask(0o077)
    with open('/tmp/makepad-carthop-ingress.lock', 'a') as lock:
        ...
        source = 'carthop-acme.conf' if args.bootstrap else 'carthop-prod.conf.template'
        rendered = {'carthop-prod.conf.template': (ROOT/'sites'/source).read_text()}
        previous_configs = spec.get('Configs', [])
        networks = ['makepad_carthop_prod_app']
        ...
        for network in networks:
            ...
            if value['Driver'] != 'overlay' or not value['Attachable'] or value['Options'].get('encrypted') != 'true':
                raise RuntimeError('CartHop requires its encrypted attachable overlays')
            required_networks.add(value['Id'])
            if value['Id'] not in previous_networks:
                additions.extend(['--network-add', network])
        with tempfile.TemporaryDirectory(prefix='carthop-ingress-') as directory:
            ...
            try:
                ...
                print('CartHop ingress deployed; existing routes, image, mounts, environment and networks preserved.')
