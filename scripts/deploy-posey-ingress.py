#!/usr/bin/env python3
"""Add Posey routes to the existing shared ingress without replacing other projects."""
import argparse
import base64
import fcntl
import hashlib
...
TARGETS = {'posey-prod.conf.template'}
...
def verify_applied(after, expected_configs, required_networks):
    ...
    if any(actual.get(target) != config_id for target, config_id in expected_configs.items()):
        raise RuntimeError('Expected Posey route was not installed')
    attached = {n['Target'] for n in after['Spec']['TaskTemplate'].get('Networks', [])}
    if not required_networks <= attached:
        raise RuntimeError('Expected Posey network was not attached')
...
def main():
    ...
    os.umask(0o077)
    with open('/tmp/makepad-posey-ingress.lock', 'a') as lock:
        ...
        source = 'posey-acme.conf' if args.bootstrap else 'posey-prod.conf.template'
        rendered = {'posey-prod.conf.template': (ROOT/'sites'/source).read_text()}
        previous_configs = spec.get('Configs', [])
        networks = ['makepad_posey_prod_app']
        ...
        for network in networks:
            ...
            if value['Driver'] != 'overlay' or not value['Attachable'] or value['Options'].get('encrypted') != 'true':
                raise RuntimeError('Posey requires its encrypted attachable overlays')
            required_networks.add(value['Id'])
            if value['Id'] not in previous_networks:
                additions.extend(['--network-add', network])
        with tempfile.TemporaryDirectory(prefix='posey-ingress-') as directory:
            ...
            for name, content in rendered.items():
                ...
                config_name = 'posey_'+name.replace('.', '_')+'_'+digest
            ...
            try:
                ...
                print('Posey ingress deployed; existing routes, image, mounts, environment and networks preserved.')
