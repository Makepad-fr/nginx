#!/usr/bin/env python3
"""Add Sentry routes to the existing shared ingress without replacing other projects."""
import argparse
import base64
import fcntl
import hashlib
...
TARGETS = {'sentry-prod.conf.template'}
...
def main():
    ...
    os.umask(0o077)
    with open('/tmp/makepad-sentry-ingress.lock', 'a') as lock:
        ...
        source = 'sentry-acme.conf' if args.bootstrap else 'sentry-prod.conf.template'
        rendered = {'sentry-prod.conf.template': (ROOT/'sites'/source).read_text()}
        previous_configs = spec.get('Configs', [])
        networks = ['makepad_sentry_prod_app']
        ...
        for network in networks:
            ...
            if value['Driver'] != 'overlay' or not value['Attachable'] or value['Options'].get('encrypted') != 'true':
                raise RuntimeError('Sentry requires its encrypted attachable overlays')
            if value['Id'] not in previous_networks:
                additions.extend(['--network-add', network])
        with tempfile.TemporaryDirectory(prefix='sentry-ingress-') as directory:
            ...
            for name, content in rendered.items():
                ...
                config_name = 'sentry_'+name.replace('.', '_')+'_'+digest
            ...
            try:
                ...
                print('Sentry ingress deployed; existing routes, image, mounts, environment and networks preserved.')
